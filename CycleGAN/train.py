...
py.arg('--crop_size', type=int, default=256)  #then crop to this size, default 256
py.arg('--batch_size', type=int, default=1)
py.arg('--epochs', type=int, default=200) #default 200
py.arg('--epoch_decay', type=int, default=100) #epoch to start decaying learning rate, default 100
...
py.arg('--identity_loss_weight', type=float, default=0.0) #default was 0.0, for photo generation from painting
py.arg('--pool_size', type=int, default=50)  # pool size to store fake samples, default is 50
args = py.args()

# output_dir
output_dir = py.join('output5-experiments/final_experiment', args.dataset)
py.mkdir(output_dir)
...
A_img_paths = py.glob(py.join(args.datasets_dir, args.dataset, 'train-CBCT'), '*.png')
B_img_paths = py.glob(py.join(args.datasets_dir, args.dataset, 'train-CT'), '*.png')
...
A_img_paths_test = py.glob(py.join(args.datasets_dir, args.dataset, 'test-CBCT'), '*.png')
B_img_paths_test = py.glob(py.join(args.datasets_dir, args.dataset, 'test-CT'), '*.png')
...
def train_G(A, B):
    with tf.GradientTape() as t:
        ...
        G_loss = A2B_g_loss + A2B2A_cycle_loss * args.cycle_loss_weight + A2A_id_loss * args.identity_loss_weight
...
'''getting sample of testset - translation is performed on test sets herer already,
 therefore running test cript not necessary!!!'''
